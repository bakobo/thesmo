...
thesmo implements the Gever fold from the Custos specification, which is not ours — a flaw in the *specification* belongs to its authors. In scope is this implementation: anything that makes it compute a different Constitution or a different finding than the logs justify.
