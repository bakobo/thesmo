...
You are **Zeta**, one implementer on the thesmo project. Read this whole brief before you
touch anything.
...
- Branch `m2-zeta`, worktree `~/code/bakobo/.worktrees/thesmo-m2-zeta`.
...
**Custos 4.2 Chapter 2 (The governed-object taxonomy)** — lines 441–861.

Build the chapter as a **total classifier**: a governed object goes in, its position on each of
the chapter's axes comes out, and every object the standard types has an answer. The chapter's
own structure is the map — the minimal case, the axes, the disclosure postures, the generated
family, and the passage from prose regime to computable domain.

Totality is the point, so the classifier needs something to be total *over*. Run it across every
governed object the document names anywhere — §12's classes above all, but also any object
introduced elsewhere that the taxonomy claims to type. Where an object has no position on an
axis, or has two, that is your finding: say which, and say whether the chapter's own text
resolves it.

The chapter states a closure test on itself, in §1.7. Where you conclude the taxonomy cannot
type something, use the vocabulary that test provides rather than inventing your own.
...
1. `docs/readings-zeta.md` — the register. Follow `docs/readings-TEMPLATE.md` in your
...
3. `cases/` — concrete cases, one JSON file each, in this shape:
   ...
   ```json
   {
     ...
     "discriminates": "one sentence: what a wrong classifier would produce instead"
   }
   ```
...
4. `python -m thesmo.<your module>.run_case <file.json>` — reads a case, prints the computed
   classification. Write cases that discriminate readings and cases that probe totality, not
   cases that pass.
5. `docs/report-zeta.md`, answering exactly these:
...
Append a timestamped line to `/tmp/thesmo-zeta-status.log` at each, and check
`/tmp/thesmo-zeta-inbox.md` for revised instructions before continuing past it:
